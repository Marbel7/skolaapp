...
(function(){
  ...
  function restoreLocalTasksIfEmpty(){
    if(!Array.isArray(window.tasks) || window.tasks.length) return false;
    try {
  ...
  function refreshTaskSummary(){
    ...
    var restored=restoreLocalTasksIfEmpty();
    ...
    if(raw){
      ...
      if(restored && typeof window.renderTasks==='function' && !window.__SKOLA_TASK_LOCAL_REPAINT__){
        window.__SKOLA_TASK_LOCAL_REPAINT__=true;
        setTimeout(function(){
          try{ window.renderTasks(); }finally{ window.__SKOLA_TASK_LOCAL_REPAINT__=false; }
        },0);
      }
      return;
    }
  ...
  function installNoteDeleteHandler(){
    ...
    list.addEventListener('click',function(e){
      ...
      if(window.fbSyncEnabled&&note.id&&typeof window.fbDeleteNote==='function') window.fbDeleteNote(note.id);
      else { window.notes.splice(idx,1); if(typeof window.saveJSON==='function')window.saveJSON('zs_notes_list_v1',window.notes); if(typeof window.renderNotes==='function')window.renderNotes(); }
    },true);
  }
  ...
  function installAccordionClick(){
    ...
    document.addEventListener('click',function(e){
      ...
      if(!target)return;
      var card=target.closest('.card'); if(!card)return;
  ...
  function taskAccordion(){
    ...
    head.replaceWith(b); expanded(card,b,false); refreshTaskSummary();
  }
  function notesAccordion(){
    ...
    head.replaceWith(b); expanded(card,b,false); refreshNotesSummary();
  }
  function styles(){
  ...
  function init(){installAccordionClick();styles();taskAccordion();notesAccordion();wrap('renderTasks',refreshTaskSummary);wrap('renderNotes',refreshNotesSummary);observe();installTaskCheckmarkObserver();installNoteDeleteHandler();refreshTaskSummary();refreshNotesSummary();syncTaskCheckmarks(document);setTimeout(function(){refreshTaskSummary();refreshNotesSummary();syncTaskCheckmarks(document);},500);}
