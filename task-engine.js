...
(function () {
  ...
  var TaskEngine = {
    ...
    init: function (uid) {
      ...
      /* index.html keeps the Firebase compat instance in a lexical `db`
         variable, not window.db. Resolve the already initialized default
         Firebase app directly instead of relying on a missing global. */
    ...
    /* Compatibility for older readers that use window.tasks. This does
       not create a second state source; it is always a reference to state. */
    ...
    /* Calendar used a legacy lexical `tasks` array. Override its global
       reader after the engine is loaded so it always reads Firestore state. */
  ...
  function installCSS() {
    ...
    s.textContent = [
      ...
      '.tck.checked{background:var(--primary,#6C5CE7)!important;border-color:var(--primary,#6C5CE7)!important;color:#fff!important;font-size:12px!important;font-weight:800!important}',
